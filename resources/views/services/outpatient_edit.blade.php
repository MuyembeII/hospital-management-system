    ...
    <section class="hero is-info is-small">
        <div class="hero-body">
            <div class="container has-text-centered">
                <p class="title">Edit OPD Visit - {{ $patient->first_name }}&nbsp;{{ $patient->last_name }} </p>
                <p class="subtitle">Update Outpatient department details</p>
            </div>
        </div>
    </section>
        ...
        <div class="container has-background-white-bis">
            <div class="columns is-multiline" data-aos="fade-in-up" data-aos-easing="linear">
                <div class="column is-8 is-offset-2">
                    ...
                    @if ($errors->any())
                        <div class="box">
                            <p class="has-text-danger">Outpatient record update failed!</p>
                    ...
                    <form action="{{ route('outpatient.update', $outpatient->id) }}" method="POST">
                        @method('PATCH')
                        @csrf
                        <input type="hidden" id="patient_id" name="patient_id" value="{{ $outpatient->patient_id}}">
                                        ...
                                        <div class="select is-fullwidth">
                                            <select name="prescription_id" id="prescription_id" class="regular-text">
                                                @foreach($medicines as $key => $value)
                                                    <option
                                                        value="{{ $value->id }}"
                                                        {{ ( $value->id == $outpatient->prescription_id) ? 'selected' : '' }}
                                                    >
                                                        {{ $value->name }}
                                                    </option>
                                                @endforeach
                            ...
                            <div class="columns">
                                <div class="column is-4 mx-2">
                                    <div class="field has-addons">
                                        ...
                                        <p class="control">
                                            <a class="button is-warning" href="{{ url('patients') }}"
                                               aria-current="page">
